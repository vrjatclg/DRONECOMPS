...
ENGAGE_CALL_ALLY_THRESHOLD = 2  # Min allies needed before transitioning from ENGAGE_CALL
...
class Drone:
    """Linear FSM: PATROL → ASSESS_THREAT → WITHDRAW → ENGAGE_CALL → DEFEND_ASSET → RTB
    ...
    def __init__(self, id, drone_type, position, communication_enabled=False, ammo_per_shot_weight=AMMO_PER_SHOT_WEIGHT, shot_damage=FIXED_SHOT_DAMAGE, health=100, fuel=5000.0):
        ...
        self.engage_call_start_time = None
    ...
    def get_protocol_status(self):
        """
        0 = PATROL 
        1 = ASSESS_THREAT/WITHDRAW/ENGAGE_CALL (Threat Detection/Response)
        2 = DEFEND_ASSET (Active Defense)
        3 = RTB (Mission End)
        4 = DESTROYED
        """
        if self.state == DroneState.PATROL:
            return 0
        elif self.state in [DroneState.ASSESS_THREAT, DroneState.WITHDRAW, DroneState.ENGAGE_CALL]:
            return 1
    ...
    def run_friendly_fsm(self, sim_time, heartbeats): 
        ...
        elif self.state == DroneState.ENGAGE_CALL:
            self.run_engage_call(sim_time, heartbeats)
    ...
    def run_assess_threat(self, sim_time, heartbeats):
        """ASSESS_THREAT → WITHDRAW if threat score high and outnumbered
    ...
    def run_withdraw(self, sim_time, heartbeats):
        """WITHDRAW → ENGAGE_CALL after withdrawing for some time
        ...
        if self.withdraw_start_time and (sim_time - self.withdraw_start_time) >= 5.0:
            self.engage_call_start_time = sim_time
            self.add_log(sim_time, "Transition", "WITHDRAW → ENGAGE_CALL: Requesting reinforcements")
            self.set_state(sim_time, DroneState.ENGAGE_CALL)
            self.withdraw_start_time = None
            return
    ...
    def run_engage_call(self, sim_time, heartbeats):
        """ENGAGE_CALL → DEFEND_ASSET when allies arrive
           ENGAGE_CALL → PATROL if threats cleared"""
        
        # Check if threats cleared
        active_enemies = [e for e in self.detected_enemies.values() if e.state != DroneState.DESTROYED]
        if not active_enemies:
            self.add_log(sim_time, "Transition", "ENGAGE_CALL → PATROL: Threats eliminated")
            self.set_state(sim_time, DroneState.PATROL)
            self.engage_call_start_time = None
            return
        ...
        # Check if reinforcements arrived
        nearby_allies = len(self.detected_friendlies)
        if nearby_allies >= ENGAGE_CALL_ALLY_THRESHOLD:
            self.add_log(sim_time, "Transition", f"ENGAGE_CALL → DEFEND_ASSET: {nearby_allies} allies responded")
            if self.known_assets:
        ...
        if self.engage_call_start_time and (sim_time - self.engage_call_start_time) % 2.0 < 0.1:
            self.add_log(sim_time, "Broadcast", f"ENGAGE_CALL active: Need backup! (Allies: {nearby_allies})")

    def check_for_engage_calls(self, heartbeats, sim_time):
        """Check if any ally is calling for help"""
        for hb in heartbeats:
            if hb['id'] == self.id:
                continue
            if hb.get('state') == DroneState.ENGAGE_CALL:
                return np.array(hb['pos'], dtype=float)
        return None
