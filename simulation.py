...
class Simulation:
    ...
    def run_step(self):
        ...
        # 4. Check for engage calls and respond (optional - if you want drones to respond to calls)
        for d in self.friendly_drones:
            if d.state in [DroneState.PATROL] and self.communication_enabled:
                call_location = d.check_for_engage_calls(friendly_heartbeats, self.sim_time)
                if call_location is not None:
                    # Drone responds to the call
                    d.add_log(self.sim_time, "Response", "Moving to assist ENGAGE_CALL")
