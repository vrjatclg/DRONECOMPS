...
class Simulation:
    ...
    def run_step(self):
        ...
        # 2. Shared Heartbeats - FIX: Include status code!
        friendly_heartbeats = []
        if self.communication_enabled:
            for d in self.friendly_drones:
                # Calculate status BEFORE broadcasting
                ...
                friendly_heartbeats.append({
                    'id': d.id,
                    'pos': d.position.copy(),  # Use copy to avoid reference issues
                    'state': d.state,
                    'status': status_str  # ADD THIS - This was missing!
                })
        ...
        if not friendlies_alive or not hostiles_alive or not asset_alive or self.sim_time > 600:
