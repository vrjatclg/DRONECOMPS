...
class Simulation:
    ...
    def run_step(self):
        ...
        # 3. AI Logic with heartbeats
        for d in self.all_drones:
            d.update_ai(self.sim_time, friendly_heartbeats)

        # 4. Check for coordinate calls and respond (optional - if you want drones to respond to calls)
        for d in self.friendly_drones:
            if d.state in [DroneState.PATROL] and self.communication_enabled:
                call_location = d.check_for_coordinate_calls(friendly_heartbeats, self.sim_time)
                if call_location is not None:
                    # Drone responds to the call
                    d.add_log(self.sim_time, "Response", "Moving to assist COORDINATE")
                    # You could change their behavior here if needed

        # 5. Movement, Firing & Logging
        for d in self.all_drones:
